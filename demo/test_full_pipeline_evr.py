...
def run_timing_test():
    log_file = os.path.join(current_dir, "pipeline_timing_log_evr_real.txt")
    ...
    try:
        ...
        log_message("System configured for Natural EVR Flow:", log_file)
    ...
    # Query selected from retrieval_test_madam_results.csv (ID: new_263)
    # This query historically triggered 'enhanced_vector' with a high score (0.8)
    # but failed 'vector_only' due to the specific term "penapisan" (filtering/screening).
    
    query = "Apa yang dimaksud dengan proses penapisan perizinan lingkungan?"
    
    log_message(f"\nTest Query: '{query}'", log_file)
    log_message("(Source: ID new_263 - Proven to trigger Enhanced Vector Retrieval naturally)", log_file)
