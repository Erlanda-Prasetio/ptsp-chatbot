...
def clear_supabase_table():
    ...
    if not supabase_url or not supabase_key:
        print("[FAIL] Missing SUPABASE_URL or SUPABASE_SERVICE_KEY environment variables!")
        return False
    ...
    print("\n" + "="*70)
    print("[DELETE]  CLEARING SUPABASE TABLE")
    print("="*70)
    ...
    if count_response.ok:
        ...
        print(f"\n[STATS] Current rows in table: {count}")
    
    # Delete all rows (using a condition that matches all rows)
    print("\n[DELETE]  Deleting all rows...")
    ...
    if delete_response.status_code in [200, 204]:
        print("[OK] Table cleared successfully!")
        print("="*70 + "\n")
        return True
    else:
        print(f"[FAIL] Failed to clear table: {delete_response.status_code}")
