"""
Lightweight enhanced utilities for rag_api.py
Improved processing without heavy dependencies
"""
...
class LightweightPDFProcessor:
    """Lightweight PDF text processing without PyMuPDF"""
...
class LightweightChunker:
    """Lightweight text chunking without NLTK"""
                ...
                for paragraph in paragraphs:
                    ...
                    if len(current_chunk) + len(paragraph) > self.chunk_size:
                        if current_chunk:
                            ...
                            # Start new chunk with simple overlap
                            sentences = self.simple_sentence_split(current_chunk)
                            overlap_text = ' '.join(sentences[-2:]) if len(sentences) >= 2 else ""
                            current_chunk = overlap_text + "\n\n" + paragraph
                        else:
                            # Paragraph is too long, split by sentences
                            sentences = self.simple_sentence_split(paragraph)
                            temp_chunk = ""
        ...
        for sentence in sentences:
            if len(current_chunk) + len(sentence) > self.chunk_size:
                if current_chunk:
                    chunks.append(current_chunk.strip())
                    # Add overlap
                    current_sentences = self.simple_sentence_split(current_chunk)
