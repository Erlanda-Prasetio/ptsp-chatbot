        ...
        with open(filepath, 'r', encoding='utf-8') as f:
            ...
            for idx, row in enumerate(reader):
                if idx >= limit:
                    break
                queries.append(row)
        print(f"✅ Loaded {len(queries)} query from {filepath}")
...
def test_madam_system(test_file: str, queries: List[Dict[str, str]], file_index: int) -> None:
    ...
    for query_idx, query_row in enumerate(queries, 1):
        ...
        try:
            print("\n⏱️  TIMING: Starting query processing (with delays)...")
    ...
    print(f"⏱️  Total Time: {total_time:.2f}s (includes delays)")
...
def main():
    ...
    print(f"   1. Delays are working correctly ✅")
    ...
    print(f"   - Actual result: {overall_elapsed:.2f}s ✅")
